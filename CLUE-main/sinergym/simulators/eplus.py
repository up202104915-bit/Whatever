...
class EnergyPlus(object):
    ...
    def __init__(
        ...
        Args:
            ...
            building_file (str): EnergyPlus input description file (.epJSON/.json/.idf).
        ...
        self._building_file = self._ensure_epjson_model(building_file)
        # Episode existed
    ...
    def _resolve_building_input(self, building_file: str) -> str:
        """Resolve building input path from absolute/relative/package data."""
        if os.path.isabs(building_file) and os.path.isfile(building_file):
            return building_file

        if os.path.isfile(building_file):
            return os.path.abspath(building_file)

        pkg_candidate = os.path.join(PKG_DATA_PATH, 'buildings', building_file)
        if os.path.isfile(pkg_candidate):
            return pkg_candidate

        raise FileNotFoundError(
            f'Building file not found: {building_file}. '
            'Use an absolute path or a file available in sinergym/data/buildings.'
        )

    def _ensure_epjson_model(self, building_file: str) -> str:
        """Convert IDF models to epJSON when necessary."""
        resolved = self._resolve_building_input(building_file)
        ...
        if suffix != '.idf':
            raise ValueError(
                f'Unsupported building model extension: {suffix}. '
                'Only .idf and .epJSON/.json are supported.'
            )

        cache_root = os.path.join(tempfile.gettempdir(), 'sinergym_idf_cache')
        os.makedirs(cache_root, exist_ok=True)
        source = Path(resolved)
        stamp = int(source.stat().st_mtime)
        out_dir = os.path.join(cache_root, f'{source.stem}_{stamp}')
        os.makedirs(out_dir, exist_ok=True)
        epjson_path = os.path.join(out_dir, f'{source.stem}.epJSON')

        if os.path.isfile(epjson_path):
            return epjson_path

        conversion_cmd = [
            os.path.join(self._eplus_path, 'energyplus'),
            '--convert-only',
            '--output-directory',
            out_dir,
            resolved
        ...
        if completed.returncode != 0:
            raise RuntimeError(
                'IDF to epJSON conversion failed.\n'
                f'Command: {" ".join(conversion_cmd)}\n'
                f'STDOUT:\n{completed.stdout}\n'
                f'STDERR:\n{completed.stderr}'
            )

        if not os.path.isfile(epjson_path):
            # Fallback: locate generated epJSON if EnergyPlus altered naming.
            candidates = list(Path(out_dir).glob('*.epJSON'))
            if len(candidates) == 1:
                epjson_path = str(candidates[0])
            else:
                raise RuntimeError(
                    'IDF conversion succeeded but epJSON output was not found '
                    f'in {out_dir}.'
                )
        return epjson_path
