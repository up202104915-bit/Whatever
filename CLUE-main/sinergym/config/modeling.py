...
class ModelJSON(object):
    ...
    def __init__(
        ...
        # JSON
        self._json_path = self._resolve_input_path(
            json_file, 'buildings')

        # EPW
        self.weather_files = [
            self._resolve_input_path(weather_file, 'weather')
        ...
        # RDD file can be passed explicitly. If not, try to infer it from
        # model path first, and then package defaults.
        if rdd_file is not None:
            self._rdd_path = self._resolve_input_path(rdd_file, 'variables')
        else:
            model_stem, _ = os.path.splitext(self._json_path)
            inferred_rdd = model_stem + '.rdd'
            pkg_rdd = os.path.join(
                self.pkg_data_path,
                'variables',
                os.path.basename(model_stem) + '.rdd')
            if os.path.isfile(inferred_rdd):
                self._rdd_path = inferred_rdd
            elif os.path.isfile(pkg_rdd):
                self._rdd_path = pkg_rdd
            else:
                self._rdd_path = None
        ...
        self.rdd_variables = None
        if self._rdd_path is not None and os.path.isfile(self._rdd_path):
    ...
    def _check_eplus_config(self) -> None:
        ...
        for w_file in self.weather_files:
            assert os.path.isfile(
                w_file), 'Weather files: {} is not available.'.format(w_file)
    ...
    def _check_observation_variables(self) -> None:
        ...
        """
        # If no RDD file is available we cannot perform variable validation.
        # This allows using custom models without pre-generated RDD.
        if self.rdd_variables is None:
            return
    ...
    def _resolve_input_path(self, path_or_name: str, pkg_subdir: str) -> str:
        ...
        Resolution order:
            ...
            3) Package data folder (sinergym/data/<pkg_subdir>)
        """
        ...
        pkg_candidate = os.path.join(self.pkg_data_path, pkg_subdir, path_or_name)
        if os.path.isfile(pkg_candidate):
            return pkg_candidate

        raise FileNotFoundError(
            f'Input file not found: {path_or_name} (looked in cwd and {pkg_subdir} package data)'
        )
